  ...
  try {
    ...
    return {
      ...
      ...(parsed.hunt && Array.isArray(parsed.hunt.targets) && Array.isArray(parsed.hunt.deals)
        ? { hunt: parsed.hunt }
        : {}),
      // schema v2 (sweep rebuild): context leads, closing calls + run stats —
      // optional, passed through verbatim so the UI sees what the pipeline wrote.
      ...(Array.isArray(parsed.leads) ? { leads: parsed.leads } : {}),
      ...(Array.isArray(parsed.closing) ? { closing: parsed.closing } : {}),
